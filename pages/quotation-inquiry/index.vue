<template>
  <div>
    <div
      class="relative lg:h-auto lg:min-h-[100vh] bg-cover bg-no-repeat"
      :style="{
      ...
      >
        ...
        >
          <div class="w-[744px] mx-auto">
            <h2 class="text-[30px] font-bold text-[#152123] mb-10 text-center">견적서 조회</h2>
            ...
            <div class="w-[650px] mx-auto mt-12 flex flex-col">
              ...
              <div v-if="totalItems === 0" class="text-center text-[#8E8D8D] mt-10">
                  조회된 견적서가 없습니다.
              </div>

              <!-- Pagination Controls -->
              <div v-if="totalItems > 0" class="flex justify-center mt-6">
                <nav class="flex items-center rounded-lg bg-white">
                  <!-- First -->
                  <button
                    @click="goToPage(1)"
                  ...
                  <button
                    ...
                    :class="[
                      'px-3 py-1 border',
                      { 'bg-[#95C3DD] text-white': currentPage === page },
                    ]"
                  >
                    {{ page }}
                  </button>
                  ...
                  <!-- Next group (...) -->
                  <button
                    v-if="paginationGroupEnd < totalPages"
                    @click="goToNextGroup"
                    class="px-3 py-1 border"
                  >
                    ...
        ...
        <div class="w-[100vh] h-[100vh] bg-white md:hidden flex flex-col justify-between">
          ...
          <div class="fixed bottom-0 left-0 right-0 lg:hidden">
            <button class="w-full custom-next-button">
              조회하기
...
const paginatedItems = computed(() => {
  ...
  return listItems.value.slice(start, start + itemsPerPage);
});

const totalPages = computed(() => {
  return Math.ceil(listItems.value.length / itemsPerPage);
});

const paginationGroupStart = computed(() => {
  return Math.floor((currentPage.value - 1) / pagesPerGroup) * pagesPerGroup + 1;
});

const paginationGroupEnd = computed(() => {
  return Math.min(paginationGroupStart.value + pagesPerGroup - 1, totalPages.value);
});

const visiblePages = computed(() => {
  const pages = [];
  for (let i = paginationGroupStart.value; i <= paginationGroupEnd.value; i++) {
    pages.push(i);
  }
  return pages;
});
...
const goToNextGroup = () => {
  const nextGroupStart = paginationGroupEnd.value + 1;
...
const goToPrevGroup = () => {
  const prevGroupStart = paginationGroupStart.value - pagesPerGroup;
  if (prevGroupStart >= 1) {
