    ...
    <main class="flex-1 lg:bg-[#F5F5F7]">
      ...
      <div class="card mx-auto lg:my-12">
        ...
        <div v-if="tab === 2" class="mb-5">
          <div class="flex items-center flex-wrap lg:justify-center space-x-4 mx-4 lg:my-4">
            <button @click="fetchFaqLao(type.fqtl_id, type.faq_type_name_kr)" v-for="(type, index) in store.faqTypeLaos"
              :key="type.fqtl_id" :class="{
          ...
          <div v-for="(faqLao, index) in store.faqLaos" :key="faqLao.faq_lao_id">
            <div @click="toggleOpenLao(index)" class="m-4 lg:m-0  cursor-pointer">
              <div class="flex items-center justify-between my-4">
                <div class="flex items-center">
                  <img src="@/assets/icons/q.svg" class="md:w-[20px] md:h-[20px] w-[16px] h-[16px]" />
                  <p class="text-[#132D5C] font-medium text-sm lg:text-xl ml-5">
              ...
              <div v-if="isOpenLao === index"
                class="border-b border-[#C0C0C0] pb-5 lg:py-4 lg:bg-white lg:border-t-2 lg:px-12 lg:overflow-auto lg:max-h-[300px]">
                <p class="text-[#152123] font-light text-xs lg:text-sm" v-html="faqLao.answer"></p>
              </div>
              <div class="border-b border-[#C0C0C0]"></div>
            </div>
          </div>
...
// Watch changes
watch([tab, IdFaq, IdFaqLao, isOpen, isOpenLao], updateUrlQuery, { deep: true });
...
onMounted(() => {
  ...
  if (queryTab === 1) {
    loadFaqType().then(() => {
      const queryIdFaq = Number(route.query.IdFaq) || 1; // default to 1
