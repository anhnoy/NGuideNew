<template>
  <div class="max-w-[1080px] bg-white">
    <div class="pt-5 mb-8 h1-custom">예약자 정보를 작성해 주세요.</div>
    <div class="w-[328px] sm:w-[600px] mx-auto">
      <!-- Tour Group -->
      <div class="items-center mt-5 sm:flex">
        <label
          for="req_group_name"
          class="lg:text-base lg:font-medium w-[145px] text-[#2F312A] text-xs font-normal"
          >관광 단체명</label
        >
        <input
          id="req_group_name"
          ...
          type="text"
          placeholder="관광 단체명을 입력해 주세요."
          class="w-full rounded input-custom lg:rounded-none"
        />
      </div>
      ...
      <div class="items-center mt-5 sm:flex">
        <label
          for="reservationName"
          class="lg:text-base lg:font-medium text-xs font-normal w-[145px] text-[#2F312A]"
          >예약자 성함</label
        >
        <input
          id="reservationName"
      ...
      <div class="items-center mt-5 sm:flex">
        <label
          for="email"
          class="lg:text-base lg:font-medium font-normal text-xs w-[145px] text-[#2F312A]"
          >이메일</label
        >
        <input
          id="email"
      ...
      <div class="items-center mt-5 sm:flex">
        ...
        <input
          ...
          class="hidden w-full rounded input-custom lg:rounded-none lg:block"
        />

        <div
          class="flex items-center w-full p-2 rounded input-custom lg:rounded-none lg:hidden"
        >
      ...
      <div
        class="text-[10px] lg:text-[13px] text-[#6EBC30] flex sm:ml-32 px-3 mt-1 lg:mt-2"
      ...
      <!-- Confirm Secret Code -->
      <div class="items-center mt-5 sm:flex lg:my-6">
        <label
          for="secretCodeConfirm"
          class="lg:text-base lg:font-medium w-[145px] text-[#2F312A] text-xs font-normal"
          >인증 번호 입력</label
        >

        <div
          class="items-center justify-between hidden w-full p-2 rounded input-custom lg:rounded-none lg:flex"
        >
          <input
          ...
          <div
            ...
            class="w-auto h-[18px] gap-1 flex items-center ml-2"
          >
            <img
            ...
            />
            <span class="w-[30px] h-[18px] text-[#E25C5C] text-[12px]">{{
              formattedTime
            }}</span>
          </div>
        </div>
        ...
        <div class="flex items-center justify-between w-full gap-2">
          ...
          <div class="flex items-center justify-end w-auto">
            <img
              ...
              src="@/assets/icons/TimeCircle.svg"
              alt="TimeCircle"
              class="w-[12px] h-[12px] mt-[5.5px]"
            />
            <span
              v-if="isCheckedOTPToKaKao && remainingTime > 0 && !OtpChecked"
              type="time"
              class="text-[12px] w-[46px] h-[18px] text-[#E25C5C] ml-[2px]"
              >{{ formattedTime }}</span
            >
            <button
              @click="
      ...
      <div
        class="text-[10px] lg:text-[13px] text-[#E25C5C] flex sm:ml-32 px-3 mt-1 lg:mt-2"
        v-if="OtpChecked === false"
      >
        인증 번호를 다시 입력해 주세요.
      </div>
      <div
        class="text-[10px] lg:text-[13px] text-[#E25C5C] flex sm:ml-32 px-3 mt-1 lg:mt-2"
        v-if="timeEndKaokaoCheck === true"
      >
        인증 시간이 초과되었습니다. 재발송을 눌러 새 인증번호를 받으세요.
      </div>

      <!-- Additional Info -->
      <div class="items-center mt-5 sm:flex lg:my-10">
        <label
          for="additionalInfo"
          class="lg:text-base lg:font-medium font-normal text-xs w-[145px] text-[#2F312A]"
          >여행 희망 사항 (선택사항)</label
        ...
        <textarea
          ...
          placeholder="견적 시 참고할 수 있도록 여행에서 희망하시는 항목이 있다면 기입해주세요."
      ...
      <div class="flex items-center justify-between mt-5 mb-3">
        <div class="flex">
          ...
          <label
            class="ml-2 lg:text-base lg:font-medium font-normal text-xs text-[#2F312A]"
          >
            <span class="text-[#E25C5C] mr-2">(필수)</span>개인정보 수집 및 이용
            동의
          </label>
        </div>
