<template>
  <div
    class="max-w-[1080px] bg-white md:h-auto h-[795px] overflow-y-auto md:overflow-hidden overflow-x-hidden"
  >
    <div
    ...
    >
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-[8px] md:gap-[10px] mb-6">
        <button
          ...
          :class="[
            ...
            store.travelCustom.selectedAccommodations === accommodation.value
              ? 'bg-theme-selected'
              : 'bg-theme-unselected',
      ...
      <div class="flex gap-[8px] flex-row justify-center">
        <h2 class="h2-custom mb-4 text-[#2F312A]">
          침대 옵션 (복수 선택 가능)
        </h2>
      </div>
      <div class="grid grid-cols-2 md:grid-cols-4 gap-[8px] md:gap-[10px] mb-6">
        <button
          v-for="bed in beds"
          :key="bed.rid"
          @click="toggleBed(bed.rid)"
          :class="[
            'custom-destination-button text-sm w-[160px] h-[44px] md:h-[40px] md:w-auto',
            isBedSelected(bed.rid)
              ? 'bg-theme-selected'
              : 'bg-theme-unselected',
          ]"
        >
      ...
      <div class="grid grid-cols-2 md:grid-cols-2 gap-[8px] md:gap-[10px] mb-6">
        <button
          ...
          :key="option.value"
          @click="setReqIncFood(option.value)"
          :class="[
            'custom-destination-button text-sm w-[160px] h-[44px] md:h-[40px] md:w-auto',
            store.travelCustom.req_inc_food === option.value
              ? 'bg-theme-selected'
              : 'bg-theme-unselected',
          ]"
        >
      ...
      <div class="grid grid-cols-3 md:grid-cols-6 gap-[10px] md:gap-[4px] mb-6">
        <button
          v-for="food in foods"
          :key="food.fdt_id"
          @click="toggleFood(food.fdt_id)"
          :class="[
            'custom-destination-button text-sm w-[101px] h-[44px] md:h-[40px] md:w-auto',
            isSelected(food.fdt_id)
              ? 'bg-theme-selected'
              : 'bg-theme-unselected',
          ]"
        >
      ...
      <div
        class="grid grid-cols-2 grid-rows-4 md:grid-cols-4 md:grid-rows-2 gap-[10px] md:gap-[10px] mb-6"
      >
        <button
          v-for="(other, index) in others"
          :key="other.value"
          @click="toggleStrictListItem(other.value)"
          :class="[
            'custom-destination-button text-sm w-[160px] h-[44px] md:h-[40px] md:w-auto',
            isStrictListItemSelected(other.value)
              ? 'bg-theme-selected'
              : 'bg-theme-unselected',
      ...
      >
        <button
          ...
          :key="vehicle.value"
          @click="store.setVehicles(vehicle.value)"
          :class="[
            'custom-destination-button text-sm w-[160px] h-[44px] md:h-[40px] md:w-auto',
            store.travelCustom.selectedVehicles === vehicle.value
              ? 'bg-theme-selected'
              : 'bg-theme-unselected',
          ]"
        >
...
const isSelected = (fdt_id) => {
  return store.travelCustom.selectedFoods.some(
