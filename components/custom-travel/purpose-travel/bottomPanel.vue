<template>
  <div class="md:w-[550px] md:h-[1165px] mx-auto rounded-xl border border-[#E6E6E6] md:border-0 md:border-transparent">
    <div>
      <div class="flex items-center p-3 md:p-0">
        <img src="@/assets/icons/person_icon.png" alt="Cate_icon" class="w-[45px] h-[45px]" />
        <h2 class="text-[#152123] text-[16px] sm:text-[30px] lg:text-[22px] font-bold ml-2 md:ml-7">
          예약자 정보
        </h2>
      </div>
      <div>
        ...
        <!-- Tour Group -->
        <input id="req_group_name" v-model="req_group_name" type="text" placeholder="관광 단체명을 입력해 주세요. (없으신 경우 성함 입력)"
        ...
        <h2 class="text-[#152123] text-[14px] lg:text-[16px] font-medium md:mt-5 md:ml-0 ml-3 mt-3">
          예약자 성함
        </h2>
        ...
        <h2 class="text-[#152123] text-sm lg:text-[16px] font-medium md:mt-5 md:ml-0 ml-3 mt-3">
          이메일
        </h2>
        ...
        <h2 class="text-[#152123] md:text-[16px] lg:text-[16px] font-medium md:mt-5 mt-3 text-sm md:ml-0 ml-3">
          통화 가능 시간
        </h2>
        <input id="calltime" v-model="req_callable_time" type="text" placeholder="통화 가능 시간을 입력해 주세요. (예시 : 오후 1시 이후)"
        ...
        <h2 class="text-[#152123] text-[14px] lg:text-[16px] font-medium md:mt-7 md:ml-0 ml-3 mt-3">
          휴대폰번호
          <span class="text-[#5E5F61] text-[10px] lg:text-[12px] font-normal">※안전한 서비스 이용을 위하여 본인 인증을 진행 합니다.</span>
        </h2>
        <!-- Phone -->
        <!-- Wrapper -->
        <!-- ✅ Correct container -->
        <div class="relative w-full md:w-[550px] md:mt-7 mt-3">
          ...
          <input v-model="phone" type="tel" maxlength="13" placeholder="휴대폰번호를 입력해 주세요."
          ...
          <button @click="phone.replace(/-/g, '').length === 11 && sendOtpToKaKao()" :class="[
          ...
          <div v-if="messageSendKaokaoCheck"
            class="absolute top-full text-[12px] lg:text-[14px] text-[#6EBC30] md:h-[14px] ml-4">
            인증 번호가 발송되었습니다.
          </div>
        </div>
        ...
        <div class="relative w-full md:w-[550px] mt-5 md:mt-8">
          <!-- Key Icon -->
          <img src="@/assets/icons/key-icon.png" alt="Key Icon"
            class="absolute top-1/2 -translate-y-1/2 w-[20px] h-[20px] opacity-70 left-5 md:left-3" />

          <!-- OTP Input -->
          <input id="OtpNumber" v-model="OtpNumber" type="text" maxlength="6"
            :disabled="!isCheckedOTPToKaKao || OtpChecked" placeholder="인증번호"
          ...
          <!-- Timer -->
          <div v-if="isCheckedOTPToKaKao && remainingTime > 0 && !OtpChecked"
            class="absolute top-1/2 left-[calc(100%-180px)] -translate-y-1/2 flex items-center gap-1">
            <img src="@/assets/icons/TimeCircle.svg" alt="Timer" class="w-[12px] h-[12px] mt-[5px]" />
        ...
        <!-- Status Messages -->
        <div v-if="OtpChecked" class="text-[10px] lg:text-[14px] text-[#6EBC30] flex mt-1 lg:mt-2 ml-4">
          인증이 완료되었습니다.
        </div>
        <div v-else-if="OtpChecked === false && OtpNumber"
          class="text-[10px] lg:text-[14px] text-[#E25C5C] flex mt-1 lg:mt-2 ml-4">
          인증 번호를 다시 입력해 주세요.
        </div>
        <div v-if="timeEndKaokaoCheck" class="text-[10px] lg:text-[14px] text-[#E25C5C] flex mt-1 lg:mt-2 ml-4">
          인증 시간이 초과되었습니다. 재발송을 눌러 새 인증번호를 받으세요.
        </div>
        <h2 class="text-[#152123] text-[14px] lg:text-[16px] font-medium md:mt-5 mt-3 md:ml-0 ml-3">
          여행 희망 사항 <span class="text-[#5E5F61] font-normal">(선택)</span>
        </h2>
        <!-- Tour Group -->
        <textarea id="additionalInfo" v-model="additionalInfo" placeholder="여행 희망 사항이 있다면 입력해 주세요.

(예시)
현지식은 꼭 먹고 싶어요
갑각류 음식은 제외해 주세요
기업 행사 진행을 위한 공간이 필요해요"
          class="rounded-lg md:w-[550px] md:h-[250px] md:rounded-lg mt-4 bg-white px-4 py-2 border border-[#E6E6E6] w-[304px] h-[200px] md:ml-0 ml-3 text-[14px] md:mt-7"></textarea>
        <div class="items-center justify-between hidden mt-5 mb-3 md:flex sm:block">
          <div class="flex">
            ...
            <label
              ...
              <span class="text-[#E25C5C] mr-2 text-[12px] md:text-[20px]">(필수)</span>개인정보 수집 및 이용 동의
            </label>
          </div>
          <div class="flex items-center cursor-pointer" @click="openModal">
            ...
            <img :src="rightIcon" alt="Privacy Policy"
  ...
  <div class="flex items-center justify-between mt-5 md:hidden">
    <div class="flex items-center">
      ...
      <label
        ...
        <span class="text-[#E25C5C] mr-2 text-[12px] md:text-[20px]">(필수)</span>
        개인정보 수집 및 이용 동의
      </label>
    </div>
    <div class="flex items-center cursor-pointer" @click="openModal">
      ...
      <img :src="rightIcon" alt="Privacy Policy" class="ml-2 cursor-pointer" />
    </div>
  </div>
</template>
