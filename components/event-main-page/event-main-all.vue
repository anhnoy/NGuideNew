<template>
  <div>
    ...
    <div v-if="filteredEvents.length > 0" class="hidden lg:block">
      ...
      <div
        v-if="isLoading"
        class="my-[40px] h-[310px] flex mx-auto justify-between"
      >
        <div
          v-for="n in 6"
          :key="n"
      ...
      <div
        ...
        class="flex justify-center items-center m-8"
      >
        <button
          @click="loadMore"
    ...
    <div class="block lg:hidden">
      <div class="w-[328px] mx-auto">
        <span
          class="flex text-[18px] font-bold text-[#152123] leading-[16px] justify-start"
        >
          오토앤투어가 추천하는 단독패키지
        </span>
      </div>
      ...
      <div
        v-else
        class="w-[328px] mt-8 grid grid-cols-1 gap-x-[8px] gap-y-[20px] mx-auto"
      >
        <div
          v-for="(item, index) in paginatedEvents"
          :key="index"
        ...
        >
          ...
          <div class="relative h-[160px] w-full">
            <img
              :src="item.ev_image"
              class="h-full w-full object-cover rounded-t-[10px]"
              alt=""
            />
          </div>
